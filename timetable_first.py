...
# 1. 목표 데이터 (워크시트: goals) — NaN 공란 통일, 수행 완료 여부(Goal*_Done) 포함
default_goals = pd.DataFrame({
    "Date": [], "Goal1": [], "Goal2": [], "Goal3": [],
    "Goal1_Done": [], "Goal2_Done": [], "Goal3_Done": []
})
...
# 2. 요일 유형 (출근일/휴일) 및 30분 단위 설정 — 워크시트: day_type (Use30Min 컬럼 포함)
default_day_type = pd.DataFrame({"Date": [], "DayType": [], "Use30Min": []})
